...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const players = await lottery.contract.methods.getPlayers().call({ from: manager });
      setPlayers(players);
    };
  ...
  return (
    <div className="App">
      <div className="container">
        ...
        <p className="manager">Managed by: <strong>{manager}</strong></p>
        <p className="players">👥 {players.length} participants | 💰 {web3.utils.fromWei(balance, 'ether')} ETH Prize Pool</p>
        <button className="winner-btn" onClick={pickWinner}>🎯 Pick a Winner</button>
        <hr />
        <p className="play-text">💸 Join the game (Min: 0.001 ETH)</p>
        <form onSubmit={play} className="play-form">
          <input className="input" placeholder="Enter amount" onChange={(e) => setValue(e.target.value)} />
          <button type="submit" className="play-btn">🚀 Deposit</button>
        </form>
      </div>
    </div>
  );
